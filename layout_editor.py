...
class LayoutEditor(QWidget):
    ...
    def _capture_display_wallpaper(self, display_index: int) -> QPixmap:
        ...
        try:
            ...
            if not display or not display.screen:
                return None
                
            # Capture from the specific display's screen object
            screen = display.screen
            
            # Get the geometry of this specific display
            geometry = screen.geometry()
            
            # Capture the screen at its position
            pixmap = screen.grabWindow(
                0,  # Window ID (0 = entire desktop)
                geometry.x(),  # X offset
                geometry.y(),  # Y offset  
            ...
            print(f"Failed to capture wallpaper for display {display_index}: {e}")
        return None
    ...
    def _update_wallpaper(self):
        ...
        # Try to capture the specific display's wallpaper
        pixmap = self._capture_display_wallpaper(self.display_manager.selected_display_index)
        
        if pixmap and not pixmap.isNull():
            # The captured pixmap should already be the right size
            # Add to scene with transparency
            self.scene.wallpaper_item = self.scene.addPixmap(pixmap)
            ...
            # Fallback: try to load wallpaper file (primary display only)
