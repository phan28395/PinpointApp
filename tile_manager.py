# pinpoint/tile_manager.py - Refactored to support design architecture

import uuid
import json
from pathlib import Path
from typing import Dict, List, Optional, Any, Tuple
from PySide6.QtCore import Signal, QObject, QTimer
from .storage import StorageManager
from .plugins.plugin_registry import get_registry
from .display_manager import get_display_manager
from .design_system import DesignSystem


class TileManager(QObject):
    """
    Enhanced tile manager that supports the new design architecture.
    Manages tiles, layouts, and coordinates between plugins and designs.
    """
    
    # Signals
    tile_updated_in_studio = Signal(dict)
    tile_config_updated = Signal(dict)
    tile_design_changed = Signal(str, str)  # tile_id, design_id
    layout_design_changed = Signal(str, dict)  # layout_id, design_settings
    
    def __init__(self):
        super().__init__()
        self.storage = StorageManager()
        self.active_live_tiles = {}
        self.shutting_down = False
        
        # Plugin and design registry
        self.registry = get_registry()
        
        # Display manager
        self.display_manager = get_display_manager()
        
        # Design system
        self.design_system = DesignSystem
        
        # Cache for data to reduce file I/O
        self._data_cache = None
        self._cache_dirty = False
        
        # Design preferences cache
        self._design_preferences = self._load_design_preferences()
        
        # Debouncing setup
        self.pending_content_updates = {}
        self.pending_config_updates = {}
        self.save_timer = QTimer()
        self.save_timer.timeout.connect(self._perform_pending_saves)
        self.save_timer.setSingleShot(True)
        
        # Update tracking
        self._update_in_progress = False
        self._last_update_source = None
        
        # Connect to registry signals
        self.registry.plugin_loaded.connect(self._on_plugin_loaded)
        self.registry.plugin_error.connect(self._on_plugin_error)
        self.registry.design_loaded.connect(self._on_design_loaded)
        
        # Initialize design system export for external designers
        self._export_design_system()

    def _load_design_preferences(self) -> Dict[str, str]:
        """Load user's preferred designs for each tile type."""
        prefs_file = Path.home() / ".pinpoint" / "design_preferences.json"
        if prefs_file.exists():
            try:
                with open(prefs_file, 'r') as f:
                    return json.load(f)
            except:
                pass
        return {}
        
    def _save_design_preferences(self):
        """Save user's design preferences."""
        prefs_file = Path.home() / ".pinpoint" / "design_preferences.json"
        prefs_file.parent.mkdir(parents=True, exist_ok=True)
        with open(prefs_file, 'w') as f:
            json.dump(self._design_preferences, f, indent=2)
            
    def _export_design_system(self):
        """Export design system for external designers."""
        export_path = Path.home() / ".pinpoint" / "design_system.json"
        export_path.parent.mkdir(parents=True, exist_ok=True)
        
        with open(export_path, 'w') as f:
            json.dump(self.design_system.export_for_designers(), f, indent=2)
            
        print(f"Design system exported to: {export_path}")

    def _on_plugin_loaded(self, tile_id: str):
        """Handle plugin loaded event."""
        print(f"Plugin loaded: {tile_id}")
        
    def _on_plugin_error(self, tile_id: str, error: str):
        """Handle plugin error event."""
        print(f"Plugin error for {tile_id}: {error}")
        
    def _on_design_loaded(self, tile_type: str, design_id: str):
        """Handle design loaded event."""
        print(f"Design loaded: {design_id} for {tile_type}")

    def _get_cached_data(self):
        """Returns cached data, loading from storage if necessary."""
        if self._data_cache is None:
            self._data_cache = self.storage.load_data()
        return self._data_cache
    
    def _save_cached_data(self):
        """Saves the cached data to storage."""
        if self._data_cache is not None and self._cache_dirty:
            self.storage.save_data(self._data_cache)
            self._cache_dirty = False

    # Core data access methods
    def get_all_tile_data(self) -> List[Dict[str, Any]]:
        """Get all tile definitions."""
        return self._get_cached_data().get("tiles", [])

    def get_tile_by_id(self, tile_id: str) -> Optional[Dict[str, Any]]:
        """Get a specific tile by ID."""
        for tile in self.get_all_tile_data():
            if tile['id'] == tile_id:
                return tile
        return None

    def get_layout_by_id(self, layout_id: str) -> Optional[Dict[str, Any]]:
        """Get a specific layout by ID."""
        for layout in self._get_cached_data().get("layouts", []):
            if layout['id'] == layout_id:
                return layout
        return None
    
    # Design-related methods
    def get_available_designs_for_tile(self, tile_type: str) -> List[Dict[str, Any]]:
        """Get all available designs for a tile type."""
        designs = self.registry.get_designs_for_tile(tile_type)
        return [
            {
                "id": design.design_id,
                "name": design.name,
                "author": design.author,
                "version": design.version,
                "preview_image": f"designs/{tile_type}/{design.design_id}.png"  # Optional
            }
            for design in designs
        ]
        
    def set_tile_design(self, tile_id: str, design_id: str):
        """Set the design for a specific tile instance."""
        tile_data = self.get_tile_by_id(tile_id)
        if not tile_data:
            return
            
        tile_type = tile_data.get('type', 'note')
        
        # Validate design exists and is compatible
        design = self.registry.get_design(tile_type, design_id)
        if not design:
            print(f"Design {design_id} not found for tile type {tile_type}")
            return
            
        # Update tile data
        tile_data['design_id'] = design_id
        
        # Save preference
        self._design_preferences[tile_type] = design_id
        self._save_design_preferences()
        
        # Mark for save
        self._cache_dirty = True
        self._save_cached_data()
        
        # Emit signal
        self.tile_design_changed.emit(tile_id, design_id)
        
        # Update live tile if active
        if tile_id in self.active_live_tiles:
            self._refresh_live_tile(tile_id)
            
    def get_preferred_design_for_type(self, tile_type: str) -> Optional[str]:
        """Get user's preferred design for a tile type."""
        return self._design_preferences.get(tile_type)
    
    # Enhanced tile creation with design support
    def create_new_tile_definition(self, tile_type: str = "note", design_id: Optional[str] = None):
        """Creates a new tile definition with optional design specification."""
        print(f"Creating new {tile_type} tile...")
        
        # Get plugin
        plugin = self.registry.get_plugin(tile_type)
        if not plugin:
            print(f"Unknown tile type: {tile_type}")
            return None
            
        # Get default configuration
        default_config = plugin.get_default_config()
        
        # Determine design to use
        if not design_id:
            # Try user preference
            design_id = self.get_preferred_design_for_type(tile_type)
            
        if not design_id:
            # Use plugin's default design
            metadata = plugin.get_metadata()
            design_id = metadata.default_design
            
        # Create new tile
        app_data = self._get_cached_data()
        new_tile = {
            "id": f"tile_{uuid.uuid4()}",
            "type": tile_type,
            "design_id": design_id,
            **default_config
        }
        
        app_data.get("tiles", []).append(new_tile)
        self._cache_dirty = True
        self._save_cached_data()
        return new_tile

    def duplicate_tile(self, tile_id: str) -> Optional[Dict[str, Any]]:
        """Create a copy of an existing tile, including its design."""
        original = self.get_tile_by_id(tile_id)
        if not original:
            return None
            
        app_data = self._get_cached_data()
        new_tile = original.copy()
        new_tile['id'] = f"tile_{uuid.uuid4()}"
        
        # Update content to indicate it's a copy
        if 'content' in new_tile:
            content = new_tile['content']
            if content:
                new_tile['content'] = f"Copy of {content}"
            else:
                new_tile['content'] = "Copy of tile"
                
        app_data.get("tiles", []).append(new_tile)
        self._cache_dirty = True
        self._save_cached_data()
        return new_tile

    # Layout management with design settings
    def create_new_layout(self, name: Optional[str] = None):
        """Create a new layout with design settings."""
        app_data = self._get_cached_data()
        
        # Get current display info
        selected_display = self.display_manager.get_selected_display()
        display_index = self.display_manager.selected_display_index or 0
        
        layout_count = len(app_data.get('layouts', []))
        new_layout = {
            "id": f"layout_{uuid.uuid4()}",
            "name": name or f"New Layout {layout_count + 1}",
            "tile_instances": [],
            "display_settings": {
                "target_display": display_index,
                "display_info": selected_display.to_dict() if selected_display else None
            },
            "design_settings": {
                "theme": "default",
                "layout_style": "free",  # free, grid, columns
                "background": "transparent",
                "padding": 0,
                "gap": 16
            }
        }
        
        app_data.get("layouts", []).append(new_layout)
        self._cache_dirty = True
        self._save_cached_data()
        return new_layout
        
    def get_layout_display_info(self, layout_id: str) -> dict:
        """Get display information for a layout."""
        layout = self.get_layout_by_id(layout_id)
        if not layout:
            return {}
            
        display_settings = layout.get("display_settings", {})
        target_display = display_settings.get("target_display", 0)
        
        # Get current display info
        display = self.display_manager.get_display(target_display)
        if display:
            return {
                "target_display": target_display,
                "display_name": display.display_name,
                "resolution": display.resolution_string,
                "saved_info": display_settings.get("display_info")
            }
        else:
            return {
                "target_display": target_display,
                "display_name": "Unknown",
                "saved_info": display_settings.get("display_info")
            }
            
    def update_layout_display(self, layout_id: str, display_index: int):
        """Update the target display for a layout."""
        app_data = self._get_cached_data()
        for layout in app_data.get("layouts", []):
            if layout['id'] == layout_id:
                if "display_settings" not in layout:
                    layout["display_settings"] = {}
                    
                layout["display_settings"]["target_display"] = display_index
                
                # Store current display info for reference
                display = self.display_manager.get_display(display_index)
                if display:
                    layout["display_settings"]["display_info"] = display.to_dict()
                    
                break
                
        self._cache_dirty = True
        self._save_cached_data()
    
    def add_tile_to_layout(self, layout_id: str, tile_id: str, x: int, y: int):
        """Add a tile instance to a layout at the specified position."""
        app_data = self._get_cached_data()
        tile_def = self.get_tile_by_id(tile_id)
        if not tile_def:
            print(f"Tile {tile_id} not found")
            return
            
        # Find the layout
        for layout in app_data.get("layouts", []):
            if layout['id'] == layout_id:
                # Create new instance
                new_instance = {
                    "instance_id": f"inst_{uuid.uuid4()}",
                    "tile_id": tile_id,
                    "x": x,
                    "y": y,
                    "width": tile_def.get("width", 250),
                    "height": tile_def.get("height", 150)
                }
                
                # Add to layout
                if "tile_instances" not in layout:
                    layout["tile_instances"] = []
                layout["tile_instances"].append(new_instance)
                
                print(f"Added tile {tile_id} to layout {layout_id} at ({x}, {y})")
                break
        else:
            print(f"Layout {layout_id} not found")
            return
            
        self._cache_dirty = True
        self._save_cached_data()  # Immediate save for structural changes
        
    def update_tile_instance_position(self, layout_id: str, instance_id: str, new_x: int, new_y: int):
        """Update the position of a tile instance in a layout."""
        app_data = self._get_cached_data()
        for layout in app_data.get("layouts", []):
            if layout['id'] == layout_id:
                for instance in layout.get("tile_instances", []):
                    if instance.get('instance_id') == instance_id:
                        instance['x'] = new_x
                        instance['y'] = new_y
                        break
                break
                
        self._cache_dirty = True
        # Debounce position updates
        self.save_timer.stop()
        self.save_timer.start(500)  # 500ms delay for position updates
        
    def update_tile_instance_size(self, layout_id: str, instance_id: str, new_width: int, new_height: int):
        """Update the size of a tile instance in a layout."""
        app_data = self._get_cached_data()
        for layout in app_data.get("layouts", []):
            if layout['id'] == layout_id:
                for instance in layout.get("tile_instances", []):
                    if instance.get('instance_id') == instance_id:
                        instance['width'] = new_width
                        instance['height'] = new_height
                        break
                break
                
        self._cache_dirty = True
        # Debounce size updates
        self.save_timer.stop()
        self.save_timer.start(500)
        
    def delete_tile(self, tile_id: str):
        """Delete a tile definition and all its instances."""
        app_data = self._get_cached_data()
        
        # Remove tile definition
        app_data['tiles'] = [t for t in app_data.get('tiles', []) if t['id'] != tile_id]
        
        # Remove instances from all layouts
        for layout in app_data.get('layouts', []):
            layout['tile_instances'] = [
                inst for inst in layout.get('tile_instances', [])
                if inst['tile_id'] != tile_id
            ]
            
        self._cache_dirty = True
        self._save_cached_data()

    # Enhanced tile projection with design support
    def project_layout(self, layout_id: str, display_index: int = None):
        """Project a layout with proper design application."""
        self.clear_live_tiles()
        layout_data = self.get_layout_by_id(layout_id)
        if not layout_data:
            return
            
        # Determine display
        if display_index is None:
            display_settings = layout_data.get("display_settings", {})
            display_index = display_settings.get("target_display", 0)
            
        self.display_manager.select_display(display_index)
        display = self.display_manager.get_display(display_index)
        
        if not display:
            print(f"Display {display_index} not found")
            return
            
        print(f"Projecting layout '{layout_data['name']}' to {display.display_name}")
        
        # Update layout's display settings
        self.update_layout_display(layout_id, display_index)
        
        # Get layout design settings
        design_settings = layout_data.get("design_settings", {})
        
        # Project each tile instance
        for instance in layout_data.get("tile_instances", []):
            self._project_tile_instance(instance, display, design_settings)
            
    def _project_tile_instance(self, instance: Dict[str, Any], display, layout_design_settings: Dict[str, Any]):
        """Project a single tile instance with its design."""
        tile_def = self.get_tile_by_id(instance['tile_id'])
        if not tile_def:
            return
            
        # Merge tile definition with instance data
        live_tile_data = {**tile_def, **instance}
        print(f"Creating tile - ID: {tile_def.get('id')} - Content: '{tile_def.get('content', '')[:30]}...'")
        # Convert coordinates
        screen_x = instance['x'] + display.x
        screen_y = instance['y'] + display.y
        
        live_tile_data['x'] = screen_x
        live_tile_data['y'] = screen_y
        
        # Get tile type and design
        tile_type = tile_def.get('type', 'note')
        design_id = tile_def.get('design_id')
        
        # Create tile with design
        tile_window = self.registry.create_tile(tile_type, live_tile_data, design_id)
        if not tile_window:
            print(f"Failed to create tile of type: {tile_type}")
            return
            
        # Apply layout-level design settings if applicable
        if hasattr(tile_window, 'apply_layout_theme'):
            tile_window.apply_layout_theme(layout_design_settings)
            
        # Connect signals
        self._connect_tile_signals(tile_window, instance['instance_id'])
        
        # Store reference
        self.active_live_tiles[instance['instance_id']] = tile_window
        tile_window.show()
        
    def _connect_tile_signals(self, tile_window, instance_id: str):
        """Connect tile signals with proper source tracking."""
        # Content changes
        tile_window.tile_content_changed.connect(
            lambda tid, content: self.update_tile_content(
                tid, content, source=f"live_tile_{instance_id}"
            )
        )
        
        # Create filtered update handler
        def create_update_handler(window, inst_id):
            def handler(tile_data):
                update_source = getattr(self, '_last_update_source', None)
                if update_source != f"live_tile_{inst_id}":
                    window.update_display_content(tile_data)
            return handler
        
        update_handler = create_update_handler(tile_window, instance_id)
        self.tile_updated_in_studio.connect(update_handler)
        tile_window._update_handler = update_handler
        
        # Config updates
        if hasattr(tile_window, 'update_display_config'):
            self.tile_config_updated.connect(tile_window.update_display_config)
            
        # Design changes
        if hasattr(tile_window, 'update_design'):
            self.tile_design_changed.connect(
                lambda tid, did: tile_window.update_design(did) if tid == tile_window.tile_id else None
            )
            
    def _refresh_live_tile(self, tile_id: str):
        """Refresh a live tile with new design."""
        # Find the tile instance
        for instance_id, tile_window in self.active_live_tiles.items():
            if tile_window.tile_id == tile_id:
                # Get updated tile data
                tile_data = self.get_tile_by_id(tile_id)
                if tile_data:
                    # Close old window
                    tile_window.close()
                    
                    # Recreate with new design
                    # This would need the full instance data and display info
                    # For now, just emit update signal
                    self.tile_config_updated.emit(tile_data)
                break

    # Editor creation with design support
    def create_tile_editor(self, tile_id: str):
        """Create an editor for a tile using the plugin system."""
        tile_data = self.get_tile_by_id(tile_id)
        if not tile_data:
            return None
            
        tile_type = tile_data.get('type', 'note')
        return self.registry.create_editor(tile_type, tile_data, self)

    # Design validation for third-party submissions
    def validate_design_submission(self, design_file: Path) -> Tuple[bool, List[str]]:
        """Validate a third-party design submission."""
        try:
            with open(design_file, 'r') as f:
                design_spec = json.load(f)
                
            # Check tile type
            tile_type = design_spec.get('tile_type')
            if not tile_type:
                return False, ["Missing 'tile_type' field"]
                
            # Validate against tile constraints
            return self.registry.validate_design_for_tile(tile_type, design_spec)
            
        except json.JSONDecodeError as e:
            return False, [f"Invalid JSON: {e}"]
        except Exception as e:
            return False, [f"Error validating design: {e}"]
            
    def install_design(self, design_file: Path) -> bool:
        """Install a validated design file."""
        # Validate first
        is_valid, errors = self.validate_design_submission(design_file)
        if not is_valid:
            print(f"Design validation failed: {errors}")
            return False
            
        # Copy to user designs directory
        user_designs = Path.home() / ".pinpoint" / "designs"
        user_designs.mkdir(parents=True, exist_ok=True)
        
        import shutil
        dest_file = user_designs / design_file.name
        shutil.copy2(design_file, dest_file)
        
        # Reload designs
        self.registry._load_designs()
        
        print(f"Design installed: {design_file.name}")
        return True

    # Cleanup methods remain the same
    def clear_live_tiles(self):
        """Clear all active live tiles."""
        for tile_window in self.active_live_tiles.values():
            try:
                tile_window.tile_content_changed.disconnect()
                if hasattr(tile_window, '_update_handler'):
                    self.tile_updated_in_studio.disconnect(tile_window._update_handler)
                if hasattr(tile_window, 'tile_config_changed'):
                    self.tile_config_updated.disconnect(tile_window.update_display_config)
            except:
                pass
            tile_window.close()
        self.active_live_tiles.clear()

    def on_app_quit(self):
        """Handle application quit."""
        self.shutting_down = True
        self.save_timer.stop()
        self._perform_pending_saves()
        
    # Rest of the methods remain largely the same...
    def update_tile_content(self, tile_id: str, new_content: str, source=None):
        """Updates tile content with debouncing."""
        self._last_update_source = source
        
        current_tile = self.get_tile_by_id(tile_id)
        if not current_tile:
            return
            
        if current_tile.get('content') == new_content:
            return
            
        if self._update_in_progress:
            return
            
        self.pending_content_updates[tile_id] = new_content
        
        app_data = self._get_cached_data()
        for tile in app_data.get("tiles", []):
            if tile['id'] == tile_id:
                tile['content'] = new_content
                updated_tile_data = tile.copy()
                break
        else:
            return
            
        self._cache_dirty = True
        
        self._update_in_progress = True
        try:
            self.tile_updated_in_studio.emit(updated_tile_data)
        finally:
            self._update_in_progress = False
            self._last_update_source = None
        
        self.save_timer.stop()
        self.save_timer.start(300)
        
    def _perform_pending_saves(self):
        """Performs all pending saves after the debounce period."""
        if self._cache_dirty:
            self._save_cached_data()
            self.pending_content_updates.clear()
            self.pending_config_updates.clear()